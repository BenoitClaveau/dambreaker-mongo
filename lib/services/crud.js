...
class CrudService {
    ...
    async connect(options = {}) {
        const mongo = await this.giveme.resolve("mongo");
        const { db, client } = await mongo.connect(this.dbName, options);
    ...
    async gridfsUploadDataURI(dataURI, filename, options = {}) {
        const match = /data:(\S*);(\S*),(.+)/.exec(dataURI);
        if (!match) throw new Error("Body is not a dataURL.");

        const contentType = match[1];
        const data64 = match[3];

        const buffer = Buffer.from(data64, "base64");

        const stream = await this.gridfsOpenUploadStream(filename, { 
    ...
    async gridfsDelete(id) {
        const bucket = await this.gridfs();
        if (!id) throw new Error("id isn't defined.");
        return bucket.delete(id);
    };

    async gridfsRename(id, name) {
        const bucket = await this.gridfs();
        if (!id) throw new Error("id isn't defined.");
