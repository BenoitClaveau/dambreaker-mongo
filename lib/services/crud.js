...
class CrudService {
    ...
    saveOne(filter, document, options, output) {
        const f = typeof filter == "function" ? filter(document) : filter;
        const o = typeof options == "function" ? options(document) : options;
    ...
    insertOne(document, options, output) {
        ...
        return streamifyOne(async () => {
            const res = await this.mongoInsertOne(document, o)
            if (typeof output == "function") return output(document, res);
            if (!res) throw new Error("Fail to insert document.", { document: d });
            return res.ops[0];
        });
    }

    insert(document, options = {}, output) {  
        return streamify(async (chunk, enc) => {
            const o = typeof options == "function" ? options(chunk) : options;
            const res = await this.mongoInsertOne(document, o);
            if (typeof output == "function") return output(document, res);
    ...
    updateOne(filter, update, options, output) {
        ...
        const o = typeof options == "function" ? options(chunk) : options;
        return streamifyOne(async () => {
            const res = await this.mongoUpdateOne(f, update, o);
            if (typeof output == "function") return output(filter, res);
    ...
    update(filter = ({_id}) => ({_id}), update, options = {}, output) {
        return streamify(async (chunk, enc) => {
            ...
            if (typeof output == "function") return output(chunk, res);
            if (!res) throw new Error("Fail to update document.", { filter: f, update: u });
            return res;
        })
    }
