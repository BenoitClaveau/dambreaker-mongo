...
const { MongoClient, GridFSBucket, Server } = require("mongodb");
...
class MongoService extends EventEmitter {
    ...
    async singleConnect(dbName = "db", options = {}) {
        ...
        return await mutex.lock(async () => { // On attend un jeton pour éviter plusieurs créations de connexion.
            ...
            this.connections[dbName] = { db, client, buckets: {}};
            client.on("close", () => {
                if (!this.connections[dbName]) return;
                for (let [buckeName, bucket] of Object.entries(this.connections[dbName].buckets)) {
                    bucket.close();
                    delete this.connections[dbName].buckets[buckeName];
                }
    ...
    async gridfs(bucketName, dbName = "db", options = {}) {
        if (this.connections[dbName]?.buckets?.[bucketName]) return this.connections[dbName].buckets[bucketName];
        const db = await this.singleConnect(dbName, options);
        const bucket = new GridFSBucket(db, { bucketName });
        this.connections[dbName].buckets[bucketName] = bucket;
        return bucket;
    }
    ...
    async close() {
        for (let [dbName, { client, buckets, db, server }] of Object.entries(this.connections)) {
            for (let [bucketName, bucket] of Object.entries(buckets)) {
                bucket.close();
            }
            await client.close();
        }
